...
pub mod nft_bank {
    ...
    pub fn transfer_into_fed_treasury_account(
        ...
        require!(
            ctx.accounts.from.owner != nft_bank::ID,
    ...
    pub fn transfer_out_of_fed_user_account(
        ...
        require!(
            ctx.accounts.from_user_vault_token_account.owner == nft_bank::ID,
            ErrorCode::InvalidFrom
        );

        // 2. To-account must be external (not Fed-owned)
        require!(ctx.accounts.to.owner != nft_bank::ID, ErrorCode::InvalidTo);
        ...
        //     Pubkey::find_program_address(&[FED_VAULT_AUTHORITY_SEED], &nft_bank::ID);
        // require!(
