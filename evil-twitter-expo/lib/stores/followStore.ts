...
export const useFollowStore = create<FollowState & FollowActions>(
  (set, get) => ({
    ...
    checkIntimateFollowStatus: async (
      targetUserId: string,
      currentUserId: string
    ) => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to check intimate follow status"
          );
        }
        const data = await response.json();
        ...
        set({
          ...
          intimateRequestStatus:
            status ?? (data.has_pending_request ? "pending" : null),
          intimateStatusLoading: false,
          intimateStatusError: null,
        });
    ...
    requestIntimateFollow: async (
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to request intimate follow"
          );
        }
        const data = await response.json();
        const status =
          normalizeRequestStatus(data.relationship_status) ?? "pending";
        ...
        console.log("intimateStatusError", error);
        set({
          intimateStatusError:
            error instanceof Error
              ? error.message
    ...
    approveIntimateFollower: async (
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to approve intimate follower"
          );
        }
        await Promise.all([
    ...
    rejectIntimateFollower: async (
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to reject intimate follower"
          );
        }
    ...
    ejectIntimateFollower: async (targetUserId: string, followerId: string) => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to eject intimate follower"
          );
        }
        await Promise.all([
          get().fetchIntimateFollowers(targetUserId),
    ...
    fetchIntimateFollowers: async (userId: string) => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/users/${userId}/intimate-followers`
        );
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to fetch intimate followers"
          );
        }
        const data = await response.json();
        set({
    ...
    fetchIntimateFollowing: async (userId: string, viewerId: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to fetch intimate following"
          );
        }
        const data = await response.json();
        set({
    ...
    fetchIntimateRequests: async (userId: string) => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Failed to fetch intimate follow requests"
          );
        }
        const data = await response.json();
        set({
