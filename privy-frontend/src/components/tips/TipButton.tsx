...
import { getTokenName } from "@/lib/utils/formatting";
...
export function TipButton({ recipientUserId, postId, size = "small", variant = "icon" }: TipButtonProps) {
    ...
    const defaultTokenMint = user?.defaultPaymentToken || BLING_MINT_STR;
    const defaultAmount = 1; // Default tip amount is 1
    const tokenName = getTokenName(defaultTokenMint, BLING_MINT_STR, USDC_MINT_STR, STABLECOIN_MINT_STR);
